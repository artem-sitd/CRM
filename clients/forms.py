...
# Проверка наличия такого номера в базе
...
class LeadUpdateForm(ClientsForm):
    to_inactive = forms.BooleanField(
        required=False, initial=False, widget=forms.CheckboxInput
    )
    to_potential = forms.BooleanField(
        required=False, initial=False, widget=forms.CheckboxInput
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        current_state = (
            self.instance.state
            if self.instance and hasattr(self.instance, "state")
            else None
        )
    ...
    def clean(self):
        ...
        if to_inactive:
            ads_hs = HistoryAds.objects.filter(client=self.instance.id).values_list(
                "id", flat=True
            )
            contracts_exist = Contract.objects.filter(
                archived=False, ads_history__id__in=ads_hs
            ).exists()
